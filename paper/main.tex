...
\revised{The corpus is then divided into training, validation and test splits of
approximately 7,412, 1,000 and 2,000 QA pairs. Because the 10,412 QA pairs are derived from
...
Table~\ref{table:dataset-composition} are preserved in each split; because a passage
cannot be divided, the realised sizes are 7,350, 1,026 and 2,036
(Table~\ref{table:dataset-splits}). Under a
...
\begin{table}[!h]
\centering
\caption{\revised{Domain-balanced dataset split composition. Sizes are what
passage-grouped stratified splitting yields; they differ from round 7{,}412/1{,}000/2{,}000
targets because whole passages cannot be divided across splits.}
\needsnum{AUTHORS: these are the sizes produced by the released splitter. The results in
Section~3 are reported on N = 2{,}000. If the reported scores come from a different
partition, replace this table with that partition's sizes; if they are regenerated with
the released splitter, change N to 2{,}036 in Section~3 and in
Table~\ref{table:performance}. The two must describe the same experiment.}}
\label{table:dataset-splits}
\begin{tabular}{lcccc}
\hline
\textbf{Domain} & \textbf{Train} & \textbf{Validation} & \textbf{Test} & \textbf{Total} \\
\hline
Taxation & \revised{1,963} & \revised{281} & \revised{556} & \revised{2,800} \\
VAT & \revised{1,855} & \revised{258} & \revised{507} & \revised{2,620} \\
Customs & \revised{2,445} & \revised{335} & \revised{668} & \revised{3,448} \\
Finance (General) & \revised{1,087} & \revised{152} & \revised{305} & \revised{1,544} \\
\hline
\textbf{Total} & \textbf{\revised{7,350}} & \textbf{\revised{1,026}} & \textbf{\revised{2,036}} & \textbf{10,412} \\
\hline
\end{tabular}
\end{table}
...
\needsnum{AUTHORS --- READ BEFORE SUBMITTING: every score in this section must come
from the passage-grouped split described in Section~2.2.1. If the submitted results were
produced by a question-level partition of the 10,412 pairs, they are inflated (93.2\% of
test items would share a source passage with training) and Tables 5, 7, 8 and 12 must be
regenerated. Delete this note once the numbers and Section~2.2.1 agree.}
