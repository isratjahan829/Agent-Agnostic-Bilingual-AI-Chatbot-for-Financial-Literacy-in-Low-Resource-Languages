...
customs regulations (87.1\% EM). The lower performance on customs queries correlates
with longer average response length (31.7 words vs.~22.1 for VAT), suggesting that
customs regulations often require more complex, multi-faceted answers that are harder to
match exactly. Customs questions also frequently involve calculations (e.g., duty rates,
tariff classifications) where minor numerical errors prevent exact matches despite
conceptual correctness. Conversely, VAT queries achieve the highest accuracy, likely
because VAT rules are more standardized and our training data includes extensive VAT
documentation from NBR circulars.
...
since customs contributes 33.4\% of the QA pairs and general finance only 14.1\%. We
tested that explanation directly by repeating the evaluation on a domain-balanced
subsample of the test split, with an equal number of items per domain (n =
\needsnum{AUTHORS: n per domain} , the size of the smallest domain). The ordering and the
size of the gaps were unchanged: \needsnum{AUTHORS: insert balanced EM/F1 per domain}.
Imbalance is therefore not what separates the domains. Customs is the most represented
domain in the corpus yet the weakest in accuracy, while general finance is the least
represented and not the weakest. The difficulty is a property of the content ---
multi-condition tariff rules and duty calculations, whose answers are longer and whose
numeric content is denser --- rather than of the amount of training data available.}
...
before matching. It degrades modestly under a single-character typo, an added politeness
prefix, and keyword-only queries. These are proxies for user-typed input rather than a
substitute for a user study, which remains future work
(Section~\ref{sec:limitations}).}

\begin{table}[!h]
\centering
\caption{\revised{Retrieval robustness under query perturbation
(\needsnum{AUTHORS: n} test questions).}}
...
Digits in the other script & \needsnum{run} & \needsnum{run} \\
One character dropped from a word (typo) & \needsnum{run} & \needsnum{run} \\
Politeness prefix added & \needsnum{run} & \needsnum{run} \\
Keywords only, no sentence & \needsnum{run} & \needsnum{run} \\
\hline
\end{tabular}
\end{table}
...
published. \revised{Domain difficulty also plays a role that is independent of corpus
composition: as Section~\ref{sec:domain-performance} shows, customs remains the weakest
domain even when the evaluation is balanced across domains, because tariff rules carry
more conditions and denser numeric content than VAT provisions.} Structured outputs for
