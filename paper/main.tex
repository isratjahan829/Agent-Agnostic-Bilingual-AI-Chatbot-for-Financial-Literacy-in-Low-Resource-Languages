...
Board of Revenue documentation, covering \revised{taxation (2,800 QAs), VAT (2,620 QAs),
customs (3,448 QAs), and finance (1,544 QAs)}. Second, Model Fine-Tuning applies QLoRA
...
\begin{table}[!h]
\centering
\caption{\revised{Dataset composition, recomputed from the released corpus.}}
\label{table:dataset-composition}
\begin{tabular}{lccccc}
\hline
\textbf{Domain} & \textbf{QA Pairs} & \textbf{\% of Dataset} & \textbf{Avg Q Length} & \textbf{Avg A Length} & \textbf{Passages} \\
\hline
Taxation & \revised{2,800} & \revised{26.9\%} & \revised{12.7 words} & \revised{25.9 words} & \revised{436} \\
VAT & \revised{2,620} & \revised{25.2\%} & \revised{17.0 words} & \revised{26.5 words} & \revised{169} \\
Customs & \revised{3,448} & \revised{33.1\%} & \revised{10.4 words} & \revised{25.1 words} & \revised{289} \\
Finance (General) & \revised{1,544} & \revised{14.8\%} & \revised{15.3 words} & \revised{22.2 words} & \revised{557} \\
\hline
\textbf{Total} & \textbf{\revised{10,412}} & \textbf{100\%} & \textbf{\revised{13.4 words}} & \textbf{\revised{25.2 words}} & \textbf{\revised{1,451}} \\
\hline
\end{tabular}
\end{table}
...
\revised{The corpus is divided into training, validation and test splits of 7,412,
1,000 and 2,000 QA pairs, stratified by domain so that the domain proportions of
Table~\ref{table:dataset-composition} are preserved in each split
(Table~\ref{table:dataset-splits}). The validation split is used for hyperparameter
selection, for calibrating the filter thresholds of
Section~\ref{sec:hallucination-method}, and for early stopping; the test split is used
only for the results reported in Section~3.}

\needsnum{AUTHORS: state the partition unit here, keeping only the sentence that is true
of the run which produced the reported scores. (a) If questions were grouped by their
source passage, write: ``Partitioning is performed at the level of the source passage:
all questions derived from a given passage, in both languages, are assigned to the same
split, so that no test item shares a source passage with the training set.'' (b) If the
10,412 pairs were partitioned individually, the reported scores are inflated and have to
be regenerated before any such sentence can be written: the corpus has only 1,451
distinct source passages, so a question-level partition places 93.2\% of test items'
source passages in the training set. Reviewer 2(a) asks this question directly, and it is
checkable from the released dataset.} Table
\ref{table:dataset-splits} presents the domain-balanced dataset composition across
training, validation, and test splits.

\begin{table}[!h]
\centering
\caption{\revised{Domain-balanced dataset split composition. Domain totals are those of
Table~\ref{table:dataset-composition}; the split sizes are 7{,}412 / 1{,}000 / 2{,}000,
allocated across domains in proportion to each domain's share of the corpus.}}
\label{table:dataset-splits}
\begin{tabular}{lcccc}
\hline
\textbf{Domain} & \textbf{Train} & \textbf{Validation} & \textbf{Test} & \textbf{Total} \\
\hline
Taxation & \revised{1,993} & \revised{269} & \revised{538} & \revised{2,800} \\
VAT & \revised{1,865} & \revised{252} & \revised{503} & \revised{2,620} \\
Customs & \revised{2,455} & \revised{331} & \revised{662} & \revised{3,448} \\
Finance (General) & \revised{1,099} & \revised{148} & \revised{297} & \revised{1,544} \\
\hline
...
lightweight training tasks. \revised{The reported fine-tuning run was performed on an
NVIDIA Tesla T4 (16\,GB) provided by Kaggle, which accommodates the 4-bit 3B model
together with the optimiser state at an effective batch size of 32.}

\revised{Table~\ref{tab:implementation} lists the complete configuration needed to
reproduce the reported results. All runs use a fixed random seed of 42. The reported
model was fine-tuned on an NVIDIA Tesla T4 (16\,GB) on Kaggle, with development and
smaller runs on the local RTX 4050 (6\,GB); training took
\needsnum{AUTHORS: wall-clock training time}. Because the T4 is a
Turing device without bfloat16 support, mixed precision is resolved from the device at
load time: float16 on the T4, bfloat16 on the RTX 4050. The quantised weights and the
LoRA adapters are identical in both cases; only the compute dtype differs.}
...
Retriever encoder & \texttt{intfloat/multilingual-e5-base} \\
...
Mixed precision & float16 on Turing (T4), bfloat16 on Ada (RTX 4050) \\
Random seed & 42 \\
\hline
\end{tabular}
\end{table}
...
\revised{The general-purpose baselines answer from the question alone: they are not
fine-tuned on this corpus and do not retrieve from the NBR documents. The comparison
therefore measures systems as they can be deployed today, not model families at equal
adaptation, and we qualify the interpretation accordingly in Section~3.11 rather than
attributing the gap to model scale.}
...
\begin{table}[!h]
\centering
\caption{Comparative performance on Bangla financial QA (N = 2,000).}
...
\revised{The ordering by accuracy is taxation, VAT, customs, general finance, while the ordering by corpus share is customs, taxation, VAT, general finance. The two do not agree: the most represented domain (customs, 33.1\% of the corpus) is not the most accurate, and the most accurate (taxation) is not the most represented. We do not draw a conclusion about which domain is weakest: the confidence intervals of general finance and customs overlap, and their ordering changed when we varied the retrieval encoder, so it is not stable enough to interpret. A controlled test of the imbalance hypothesis would require retraining on a domain-balanced \emph{training} set rather than re-balancing the test split --- per-domain accuracy does not depend on the other domains' test sizes --- and we identify that as future work.}
...
response latency (450 ms) is also lower than the cloud-hosted alternatives. Table~\ref{tab:baseline-comparison} provides a
...
fabricated answers by 8.0 percentage points). \revised{The gap to the general-purpose baselines should be read as evidence about what
a system has access to rather than about model scale: BanglaFinGPT is adapted to this
domain and retrieves from the documents the answers are drawn from, and the baselines do
neither. It is not evidence that a 3B model is intrinsically stronger than a larger one,
and we no longer make that claim.}
...
proposals, so the benchmark inherits phrasing conventions from both. A study with real
user queries would be needed to measure how far that limits external validity.}
...
\revised{\url{https://github.com/isratjahan829/Agent-Agnostic-Bilingual-AI-Chatbot-for-Financial-Literacy-in-Low-Resource-Languages}}. Model weights (the trained LoRA adapters) are available from
